...
**Quantum Logical Framework (QLF)** is a formal proof system machine-verified in Lean 4 across **218 modules with zero `sorry` blocks**. It encodes quantum mechanics and spacetime dynamics using phase-string combinatorics (ZFA — Zero-phase Flux Algebra).
...
## Modules — 218, machine-verified, zero `sorry`

Registered in `lakefile.lean` roots array (build order); sources in `lean/`. **The full per-module table — descriptions + key-theorem lists for all 218 — lives in [`lean/README.md`](lean/README.md); consult it when working on any specific module.** Thematic families and every individual result are also mapped in [`FlowChart.md`](FlowChart.md). The core anchors a session references most often:
...
| `lakefile.lean` | Build config; `roots` array lists all 218 modules |
...
| `compression_census.py` / `Data_Compression.md` / `data/compression_census.json` | **Can QLF compress? The theory yes, a codec no — and a closure is barely compressible.** Three codes on the substrate's own object (a ZFA closure vs raw `3 bits/twist`): the enumerative floor `log₂ W_L / L`, the streaming **cylinder-prime code** (factor into primes, each costs `3|π| + log₂ M`), and **seed + `/solve`** (store a prefix, the selection cascade regenerates the rest). Exact to `L = 200`, checked against every closure to `L = 8`. Result: both rates climb to `3` — a count-balanced history is a **closed walk on `ℤ⁴`**, so `W_L ≈ (8/π²)·8^L/L²` (measured `0.8065` vs `0.8106`) and a closure is compressible by only **`2·log₂ L + 0.3` bits in total**; the prime Kraft mass `Σ_π 8^{−|π|}` **is Pólya's `ℤ⁴` return probability** `p₄ = 0.1932` (extrapolated `0.19316`), so every closure event is worth `2.37 bits` and, the walk being transient, a closure has `≈ 1.5` prime factors at any length. `/solve` regenerates 54 % of length-6 primes from two twists short, 12 % from three: **you store only the exceptions**. **Pinned at `L = 1000` (36 s): `M(∞) = 0.1932015` vs `p₄ = 0.1932017`. **Corrects the record:** `intermittency_bridge.py`'s `M(∞) = 0.18267` was converged in `R` but truncated at `L = 22`; the limit is `p₄`. Nothing downstream changes |
| `doob_bridge.py` / `data/doob_bridge.json` | **The census as a diffusion model — sample closures proportional to ways.** A balanced history is the `ℤ⁴` walk conditioned to return to the origin; conditioning on the endpoint is **Doob's h-transform**, so stepping with probability `h(x+e,t−1)/h(x,t)` (`h` = ways to close, exact via the `ℤ²×ℤ²` split, integer draws only) gives a sampler **uniform over the `W_L` closures by telescoping** — verified deterministically (every sample's step-probability product is exactly `1/W_L`), empirically (20k samples at `L=6`: strata `χ²=0.6`/6 dof, per-closure `5210`/5119 dof) and at scale (factor means within 1.6 s.e. of the exact `F_L/W_L` to `L=100`). `∇log h` **is the score** a diffusion model learns; here it is `−4x/t` exactly (measured). `/solve` is its argmax (mode), this is its sample; rejection would need `π²L²/8` strings per closure (12,500 at `L=100`), the bridge none. `Data_Compression.md` §6: lossy compression = many-to-one closure at `log 2` per dropped exception; **not** a compressor of external data |
