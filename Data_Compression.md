...
Script: [`compression_census.py`](compression_census.py); record: [`data/compression_census.json`](data/compression_census.json).
...
**Counted layer, exact integers to `L = 200`** (`W_L` from `balanced_history_count`; primes `I_L` from the Dyson
...
- **`W_L ≈ (8/π²)·8^L / L²`.** The return probability of the `d`-dimensional simple walk at time `L` is
  `2 (d/2πL)^{d/2}`; at `d = 4` that is `8/(π² L²)`. Measured: `W_L·L²/8^L = 0.8065` at `L = 200`, against
  `8/π² = 0.8106`. So a length-`L` closure is compressible by exactly
...
- **The prime Kraft mass is Pólya's return probability.** `Σ_π 8^{−|π|}` is, letter for letter, the probability
  that the `ℤ⁴` walk ever returns to the origin. Exact partial sum `M(200) = 0.191911`; the tail decays as `1/L`
  (first-return probability `~ L^{−2}`), and one Richardson step gives `M(∞) ≈ 0.19316`, against Pólya's
  `p₄ = 0.193201673…` (Pólya 1921; value in Finch, *Mathematical Constants* §5.9). **Every closure event is
...
- **Why the mean number of factors is bounded.** The `ℤ⁴` walk is *transient* (Pólya's theorem: recurrent for
  ...
  the mean number of prime factors peaks at `1.607` (`L = 12–16`) and falls toward `≈ 1.49` (extrapolated;
  consistent with the bridge value `(1 + p₄)/(1 − p₄) = 1.479`, *conjectural identification*). A long closure is
...
`0.19320…` or converge to another value, §3 is wrong. Extending `--dp-len` is cheap (`L = 200` in 45 s, exact
integers); the tail is `≈ 0.12/L`, so `L = 1000` would pin the fourth decimal.
