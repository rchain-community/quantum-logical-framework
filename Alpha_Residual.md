...
first-closure Kraft mass is converged to `M(∞) = 0.18267…` (converged in `R`; truncated at `L = 22` — the length limit is Pólya's `ℤ⁴` return probability `0.1932`, see [`Closure_Walk.md`](Closure_Walk.md) §1; the `W(R)` profile below is unaffected). The per-octave flux multiplier
