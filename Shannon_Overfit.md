...
**Measured.** How much redundancy the primes actually expose is now a number — [`Data_Compression.md`](Data_Compression.md): a closure is compressible by only `2·log₂ L + 0.3` bits in total (the census is a closed `ℤ⁴` walk, `W_L ≈ (8/π²)8^L/L²`), and each closure event is worth `−log₂ p₄ = 2.37` bits with `p₄` Pólya's return probability — the incompressible generators are *almost everything*.
